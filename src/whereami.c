...
#endif
#include <windows.h>
#include <intrin.h> // NOTE: To be commented out in case there is a compilation problem
#if defined(_MSC_VER)
#pragma warning(pop)
#endif
...
int WAI_PREFIX(getExecutablePath)(char* out, int capacity, int* dirname_length)
{
  ...
  for (;;)
  {
    resolved = (char*)realpath(WAI_PROC_SELF_EXE, buffer);
    if (!resolved)
      break;
    ...
    for (;;)
    {
      ...
      if (sscanf(buffer, "%" PRIx64 "-%" PRIx64 " %s %" PRIx64 " %x:%x %u %s\n", &low, &high, perms, &offset, &major, &minor, &inode, path) == 8)
      {
        ...
        if (low <= addr && addr <= high)
        {
          ...
          resolved = (char*)realpath(path, buffer);
          if (!resolved)
            break;
